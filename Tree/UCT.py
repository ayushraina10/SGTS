...
class UCT():
    ...
    def simulate_single_step(self):
        ...
        if need_expansion:
            ...
            next_state, reward, done = self.wrapped_env.step(expand_action, state = curr_node.state)
            # self.checkpoint_data_manager.checkpoint_env("main", self.global_saving_idx)
            ...
            # self.checkpoint_data_manager.load_checkpoint_env("main", curr_node.checkpoint_idx)
            next_state, reward, done = self.wrapped_env.step(action, state = curr_node.state)
        ...
        while not done:
            ...
            next_state, reward, done = self.wrapped_env.step(action, state = next_state)
